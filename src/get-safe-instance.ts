...
function getSafeMethodFactory<T extends object>(
	...
	return (method: PromiseMethods<T>) => {
		const getBufferCb = new (CircuitBreakerCls as typeof CircuitBreaker)(
			raceFactory(
				timeout,
				(source[method as unknown as keyof T] as Function).bind(source),
			),
			breakerOptions,
		);
...
export type PromiseMethods<T extends object> = keyof PickMatching<
	T,
...
export function getSafeInstance<T extends object>(
	...
	const map = new Map(
		methods.map(getSafeMethodFactory(timeout, source, breakerOptions, onError)),
	);
